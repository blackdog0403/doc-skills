...
description: "Verify factual claims in technical documents against authoritative sources. Remove or flag third-party unverifiable data, validate claims against official AWS documentation and announcements, add citations, and produce a correction report. Use for requests such as 'fact check this', 'verify the claims', 'check sources', 'remove third-party data', or 'add citations'. Equivalent requests written in Korean are also supported."
icon: "🔍"
