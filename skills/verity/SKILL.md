...
name: verity
description: |
  Verify factual accuracy of AI-generated responses. Decomposes responses into
  atomic claims, checks grounding against retrieved sources (AWS docs, account
  data, API results), and produces a FARM trust score.
  Use when: "verify this", "is this accurate", "fact check my answer",
  "trust score", "grounding check", or equivalent requests written in Korean.
...
```
/verity <response_text|"last response"> [depth:quick|standard|deep] [threshold:0.6] [output:summary|card|json]
```

- **response_text** (required): The AI-generated response to verify, or "last response" to auto-verify the previous answer
- **depth** (optional, default: "quick"):
  - `quick` — source check only
  - `standard` — full pipeline
  - `deep` — multi-source triangulation
- **threshold** (optional, default: 0.6): Minimum FARM score to pass (0.0–1.0)
- **output** (optional, default: "summary"):
  - `summary` — inline markdown (renders everywhere)
  - `card` — visual HTML artifact
...
### Step 3: Check Grounding Sources
...
**Active verification:** When grounding is unclear, use `WebSearch` or `WebFetch` to check against:
...
#### `card` format (HTML artifact)

Generate an HTML artifact with:
- Visual trust meter (green/yellow/orange/red)
...
- Use active verification (WebSearch/WebFetch) when grounding is unclear
